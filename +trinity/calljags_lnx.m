...
cleanupHandle = trinity.move_to_wdir(options);
...
%    end
    delete(stream)
    trinity.set_permissions('+x', scriptfile{ch});
end
...
libpath = trinity.preferences('libpath_lnx');
...
if doparallel
    ...
    [fid, stream] = trinity.robust_fopen(tfn, 'w');
    ...
    if status
        parsed = trinity.parse_jags_errors(options, result);
        trinity.error_tag('trinity:calljags_lnx:launch_jags:errorcallingparallel',...
            'System threw error:\n%s\n%s', result, parsed)
    end
    ...
    for iChain = 1:nchains
        cmd = sprintf('export LD_LIBRARY_PATH=%s; jags %s', ...
        ...
        if status{iChain}
            warning('trinity:calljags_lnx:launch_jags:errorcallingjags',...
                'System threw error:\n%s', result{iChain})
        end
    ...
    if any([status{:}])
        trinity.error_tag('trinity:calljags_lnx:launch_jags:errorcallingjags',...
            'System threw errors:\n%s\n', result{[status{:}]})
    end
end
...
if doparallel
    ...
    [fid, stream] = trinity.robust_fopen(filenm, 'wt');
...
if doparallel
    ...
    function error_parser(resultnow)
        ...
        if ~isempty(errstr)
            ...
            trinity.error_tag('trinity:calljags_lnx:error_checking:jagsError', ...
        ...
        if showwarnings ~= 0
            ...
            errstr = regexpi(resultnow, pattern, 'match');
            if ~isempty(errstr)
                warning('trinity:calljags_lnx:error_checking:jagsWarning', ...
