function options = prechecks(options)
...
options.workingdir = trinity.get_full_path(workingdir);
...
switch class(model)
    ...
    case 'cell'  % ... or we need to write the model to a temp file
        modelfilename = trinity.untitled(options, 'model');
        trinity.cell2model(model, modelfilename)
end

assertunlocked(modelfilename, 'file');

trinity.set_permissions('+x', modelfilename);

options.model         = trinity.get_full_path(modelfilename);
...
switch class(data)
    ...
    case 'struct'  % ... or we need to write the model to a temp file
        datafilename = trinity.untitled(options, 'data');
        trinity.str2data(engine, datafilename, data)
end

assertunlocked(datafilename, 'file');

trinity.set_permissions('+x', datafilename);

options.data  = trinity.get_full_path(datafilename);
...
for c = 1:nchains
    switch class(init{c})
        ...
        case 'struct'  % ... or we need to write the inits to a temp file
            initfilename_upd{c} = trinity.untitled(options, 'init', c);
            trinity.str2data(engine, initfilename_upd{c}, init{c})
    end
    
    assertunlocked(initfilename_upd{c}, 'file');
    
    trinity.set_permissions('+x', initfilename_upd{c});
end

options.init = cellfun(@trinity.get_full_path, initfilename_upd, 'uni', 0);
...
options.logfilename = cellfun(@trinity.get_full_path, logfilename_upd, 'uni', 0);
...
if exist(folderName, 'dir') == 7
    ...
    if s == 1
        ...
        if status ~= 1
            warning('trinity:prechecks:iswritable:undeletable', ...
                'Test folder "%s" could be written but not be deleted', ...
                fullfile(folderName, testDir));
        end
    end
else
    trinity.error_tag('trinity:prechecks:iswrite:wdirmissing', ...
...
model = trinity.model2cell(modelfilename);

if ~allowunderscores
    underscores = cellfun(@(x)any(x=='_'), model);
    
    if any(underscores)
        trinity.error_tag('trinity:prechecks:grammar_check:illegal_character', ...
