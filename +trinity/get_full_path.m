...
%
% FullName = GET_FULL_PATH(Name, Style)
% INPUT:
...
%   limit is exceeded. Then "dir(get_full_path([folder, '\*.*], 'fat'))" helps.
%
% EXAMPLES:
...
%   get_full_path('.', 'fat')         % '\\?\C:\Temp\File.Ext'
%
% COMPILE:
%   Automatic: InstallMex get_full_path.c uTest_get_full_path
%   Manual:    mex -O get_full_path.c
...
% - Mex calls Windows API function get_full_path.
% - Mex is much faster.
...
% Handle cell strings:
% NOTE: It is faster to create a function @cell\get_full_path.m under Linux, but
% under Windows this would shadow the fast C-Mex.
if isa(File, 'cell')
   for iC = 1:numel(File)
      File{iC} = trinity.get_full_path(File{iC}, Style);
   end
   return;
end
...
if isempty(hasDataRead)
   ...
   %      ['get_full_path: Using slow Matlab-version instead of fast Mex.', ...
   %       char(10), 'Compile: InstallMex get_full_path.c']);
   %end
...
if isempty(File)  % Accept empty matrix as input:
   if ischar(File) || isnumeric(File)
      ...
      trinity.error_tag(['trinity:JSimon:', mfilename, ':BadTypeInput0'], ...
         'Input must be a string or cell string.');
   end
end

if ischar(File) == 0  % Non-empty inputs must be strings
   trinity.error_tag(['trinity:JSimon:', mfilename, ':BadTypeInput1'], ...
      'Input must be a string or cell string.');
end
   ...
   if isUNC == 0                        % File is not a UNC path
      ...
      elseif FileLen == 2 && File(2) == ':'   % "C:" current directory on C!
         ...
         if strncmpi(ThePath, File, 2)
            ...
            try
               File = cd(cd(File));
            catch me   % No MException to support Matlab6.5...
...
if isWIN
   if ~ischar(Style)
      trinity.error_tag(['trinity:JSimon:', mfilename, ':BadTypeInput2'], ...
         'Input must be a string or cell string');
   end
