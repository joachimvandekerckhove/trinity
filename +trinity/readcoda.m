function out = readcoda(wdir, sfns, verbosity)
% READCODA  Reads a BUGS or JAGS CODA file to a MATLAB structure
%   OUT = READCODA(FILENAME, [CHAIN NUMBER], [VARIABLE TO SKIP]), will
...
if exist(indf{1}, 'file')
    [fid, stream] = trinity.robust_fopen(indf{1}, 'r');
else
...
for chain = 1:numel(codf)
    [fid, stream] = trinity.robust_fopen(codf{chain}, 'r');
