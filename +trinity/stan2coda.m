...
elseif iscell(options)
    ...
    trinity.error_tag('trinity:stan2coda:badInput', ['STAN2CODA accepts as input either an' ...
        ' options structure or a cell list of coda file names.'])
end

% Find all header lines
if ~exist(coda_files{1}, 'file')
    trinity.error_tag('trinity:stan2coda:stansamplesfilemissing', ...
        'Samples file %s not found. Bailing out.', coda_files{1})
end

[fid, stream] = trinity.robust_fopen(coda_files{1}, 'r');
...
if isempty(alllabels)
    trinity.error_tag('trinity:stan2coda:stansamplesmissing', ...
