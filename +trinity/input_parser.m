function options = input_parser(engine, varargin)
...
d = struct(...
    'engine'           , 'stan' , ...         Stan internal defaults:
    'remake'           ,     0  , ...         -----------------------
    'seed'             ,    []  , ...                      from clock
...
if check && ~isempty(unknown)
    fprintf('Unknown field(s) for %s: %s\n', upper(engine), ...
        sprintf('\n- %s', unknown{:}));
    trinity.error_tag('trinity:input_parser:updatestruct:unknownFields', ...
...
n = length(varargin);
if (mod(n, 2))
    trinity.error_tag('Each option must be a string/value pair.');
end

% Check the number of supplied output arguments
if (nargout < (n / 2))
    trinity.error_tag('Insufficient number of output arguments given');
elseif (nargout == (n / 2))
    warn = 1;
    nout = n / 2;
else
