...
% This file is only temporarily included in Trinity. \jv
...
if wine
    scriptFile = [fileparts(Bugdir),filesep,'script.txt'];
else
    scriptFile = [fileparts(Bugdir),'\','script.txt'];
end
...
if openBUGS
  ...
  fprintf(fid, 'refresh(%u) \n', refreshrate); % Andrew Jackson - line added \jv moved
...
if ischar(dataStruct) % nothing to do but copy
    copyfile(dataStruct, fileName, 'f')
    return
end
if iscell(dataStruct) && numel(dataStruct)==1 % nothing to do but copy
    copyfile(dataStruct{1}, fileName, 'f')
    return
end
