...
standir = trinity.preferences('stan_main_dir');
...
if status || ~isempty(strfind(output, 'FAILURE'))
    trinity.error_tag('trinity:callstan_lnx:makeerror', ...
...
if status
    trinity.error_tag('trinity:callstan_lnx:stanerror', ...
        'System threw Stan error: %s', output)
end

options.result = output;
options.coda_files = cellfun(@trinity.get_full_path, coda_files, 'uni', 0);
...
if doparallel
    for iChain = 1:nchains
        ...
        if fid == -1
            trinity.error_tag('trinity:callstan_lnx:save_stan_output:fileOpenErrorPar', ...
                message);
        end
    ...
    for iChain = 1:nchains
        [fid, message] = fopen(logfilename{iChain}, 'wt');
        if fid == -1
            trinity.error_tag('trinity:callstan_lnx:save_stan_output:fileOpenError', ...
                message);
        end
%         keyboard
...
%% ----------------------------------------------------------------- %%
    function error_parser(resultnow)
        ...
        if ~isempty(errstr)
            ...
            trinity.error_tag('trinity:callstan_lnx:error_checking:stanerror', ...
