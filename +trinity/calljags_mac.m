...
libpath = trinity.preferences('libpath_mac');
...
if doparallel
    ...
    if status
        trinity.error_tag('trinity:calljags_mac:errorcallingparallel',...
            'System threw error:\n%s', result)
    end
    ...
    if any([status{:}])
        trinity.error_tag('trinity:calljags_mac:errorcallingjags',...
            'System threw errors:\n%s\n', result{[status{:}]})
    end
end
...
if doparallel
    ...
    function error_parser(resultnow)
        ...
        if ~isempty(errstr)
            ...
            trinity.error_tag('trinity:calljags_mac:error_checking:jagsError', ...
