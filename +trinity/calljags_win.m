...
% libpath = trinity.preferences('libpath_win');
...
if doparallel
    ...
    if any([status{:}])
        trinity.error_tag('trinity:calljags_win:errorcallingparalleljags',...
            'System threw errors:\n%s\n', result{[status{:}]})
    end
    ...
    if any([status{:}])
        trinity.error_tag('trinity:calljags_win:errorcallingjags',...
            'System threw errors:\n%s\n', result{[status{:}]})
    end
end
...
if doparallel
    ...
    function error_parser(resultnow)
        ...
        if ~isempty(errstr)
            ...
            if erroronerror
                trinity.error_tag('trinity:calljags_win:error_checking:jagsError', ...
