function out = get_matrix_from_coda(coda, target, fcn)
% GET_MATRIX_FROM_CODA  Extract statistics of a matrix of posteriors
%   MTX = GET_MATRIX_FROM_CODA(CODA, TARGET, [FUNCTION]), where CODA is 
%   a coda structure, TARGET is a string with the name of a matrix-valued
...
%     MTX = GET_MATRIX_FROM_CODA(CODA, 'MU', @std) will return an M-by-N
...
[selection, n_sel] = trinity.select_fields(coda, ['^' target '_[0-9]']);
